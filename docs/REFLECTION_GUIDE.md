...
```go
// 定义结构体，推荐使用 eit_db tag（避免与其他库的 db tag 冲突；db 仍兼容）
type User struct {
...
### 2. Tag 语法（eit_db 推荐，db 兼容）

```go
`eit_db:"column_name,option1,option2,..."`
```
...
- `default=value` - 默认值（例如 `default=true`、`default=18`、`default=guest`）
- `type=value` - 覆盖推导类型（例如 `type=json`、`type=decimal`）
- `-` - 忽略此字段（不映射到数据库）

说明：
- 解析优先级：`eit_db` > `db` > `gorm`（兜底提取 column/主键/非空/唯一/索引/自增/默认值）。
- 历史代码可以继续使用 `db`，建议新代码统一迁移到 `eit_db`，以减少多库并存时的 tag 命名冲突风险。

兼容策略时间线：
- `v1.x`：`db` tag 继续兼容（不推荐新增）。
- `v1.x`：新功能与示例统一以 `eit_db` 为默认。
- `v2`：将评估移除 `db` tag 兼容层，最终以正式迁移公告为准。

**示例：**
```go
type Product struct {
    ID          int     `eit_db:"id,primary_key,auto_increment"`
    SKU         string  `eit_db:"sku,not_null,unique,index"`
    Name        string  `eit_db:"name,not_null"`
    Price       float64 `eit_db:"price,not_null"`
    Stock       int     `eit_db:"stock"`
    IsAvailable bool    `eit_db:"is_available,not_null"`
    Internal    string  `eit_db:"-"` // 不映射到数据库
}
```
...
type User struct {
    Name  string  `eit_db:"name,not_null"`
    Age   *int    `eit_db:"age"`            // 使用指针表示可空
    Email *string `eit_db:"email"`          // 可空
}

// 或使用 sql.Null* 类型
type User struct {
    Name  string         `eit_db:"name,not_null"`
    Age   sql.NullInt64  `eit_db:"age"`
    Email sql.NullString `eit_db:"email"`
}
```
...
type User struct {
    ...
    Username string `eit_db:"username"`
    Email    string `eit_db:"email"`
}
...
type Article struct {
    ID        int       `eit_db:"id,primary_key,auto_increment"`
    Title     string    `eit_db:"title,not_null"`
    Content   string    `eit_db:"content,not_null"`
    Author    string    `eit_db:"author,not_null"`
    ViewCount int       `eit_db:"view_count"`
    Published bool      `eit_db:"published,not_null"`
    CreatedAt time.Time `eit_db:"created_at"`
}
...
type Profile struct {
    ID        int     `eit_db:"id,primary_key"`
    UserID    int     `eit_db:"user_id,not_null"`
    Bio       *string `eit_db:"bio"`        // 可空
    Website   *string `eit_db:"website"`    // 可空
    AvatarURL *string `eit_db:"avatar_url"` // 可空
}
...
// ✅ 推荐：使用 snake_case
type User struct {
    FirstName string `eit_db:"first_name"`
}

// ⚠️ 不推荐但支持：驼峰会自动转换
type User struct {
    FirstName string `eit_db:"FirstName"` // 自动转为 first_name
}
...
```go
type Legacy struct {
    ID int `eit_db:"UserID"`  // 映射到 UserID 列
}
```
...
```go
type User struct {
    Password string `eit_db:"-"`  // 不映射到数据库
    Internal string `eit_db:"-"`
}
```
...
type User struct {
    Profile struct {
        Bio string
    } `eit_db:"profile"` // 存储为 JSON
}
```

**Q: 如何与事务结合？**
```go
// 业务层推荐：使用 WithChangeset 封装事务写入
err := repo.WithChangeset(ctx, userSchema, func(executor *db.ChangesetExecutor) error {
    cs := db.NewChangeset(userSchema)
    cs.Cast(map[string]interface{}{"email": "alice@example.com"}).Validate()
    if !cs.IsValid() {
        return fmt.Errorf(cs.ErrorString())
    }
    _, err := executor.Insert(cs)
    return err
})

// 底层场景（迁移/框架集成）才建议直接 Begin
tx, _ := repo.Begin(ctx)
defer tx.Rollback(ctx)

rows, _ := tx.Query(ctx, "SELECT * FROM users FOR UPDATE")
_ = rows
