...
# EIT-DB v1.0.3 Release Notes

> 发布日期：2026-03-20  
> 类型：Patch（Migration Policy Update）

---

## 概述

v1.0.3 在 v1.0.2 的 PostgreSQL 修复基础上，进一步收敛迁移工具入口，统一到 SchemaMigration 与 RawSQLMigration 双路径，降低后续方言回归风险。

---

## 主要变更

### 1) 迁移策略收敛：仅保留 Schema + Raw SQL 两种入口

- SchemaMigration 作为默认迁移入口。
- RawSQLMigration 保留为高级入口，用于数据库方言专属语法。

### 2) RawSQLMigration 强制 adapter 绑定

- Raw SQL 迁移执行前必须调用 `ForAdapter(...)`。
- 绑定 adapter 必须与当前 Repository adapter 一致，否则直接失败。
- 防止 SQL 在错误数据库执行。

### 3) v1 Migrator 入口禁用

- v1 `Migrator` 的 `Register/Up/Down/Status` 已禁用并返回明确引导错误。
- 统一引导到 `MigrationRunner`。

### 4) 文档更新

- 新增 `docs/MIGRATION_TOOL_GUIDE_v1_1.md`，明确迁移新策略与执行规范。

---

## 回归验证

- `TestRawSQLMigration_RequiresForAdapter`
- `TestRawSQLMigration_AdapterMustMatchCurrentRepo`
- `TestMigratorV1EntryDisabled`
- `TestCompileSQLMigrationOperation_*`
- `TestMigrationLogPlaceholder_*`
- `TestBuildCreateTableSQL_*`

---

## 升级方式

go get github.com/eit-cms/eit-db@v1.0.3

该版本不引入新的公共 API，但迁移入口策略更严格，建议升级后优先检查自定义 Raw SQL 迁移是否已绑定 `ForAdapter(...)`。
