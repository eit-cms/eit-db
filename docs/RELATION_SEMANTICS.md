...
| 后端 | 支持等级 | 当前落地能力 | 备注 |
|---|---|---|---|
| Neo4j | 强支持 | 关系名映射边类型；ManyToMany + Through 编译为两跳关系路径 | 图模型原生表达 |
| SQL Server | 强支持 | ManyToMany 策略可配置：direct_join / recursive_cte（含深度与 MAXRECURSION） | 关系路径可策略化 |
| MongoDB | 中-强支持 | lookup / pipeline 可配置；ManyToMany 两段 lookup；through 临时字段可折叠 | 聚合管线模拟关系语义 |
| PostgreSQL | 弱支持（当前） | 以关系型 join 为主；不提供图语义增强基线 | 可执行关系查询，但非图语义一等能力 |
| MySQL | 弱支持（当前） | 以关系型 join 为主；不提供图语义增强基线 | 可执行关系查询，但非图语义一等能力 |

## 2. 为什么 PostgreSQL / MySQL 是“弱支持（当前）”

弱支持不代表“不能用”，而是表示在当前版本中：

1. PostgreSQL / MySQL 主要承担关系型 join 执行，而不是图语义（边类型、路径遍历、关系级优化）的一等实现。
2. 递归 CTE / 应用层拼装可覆盖部分场景，但在语义一致性、可移植性和治理成本上弱于图后端或专项策略后端。
3. 项目目标是多适配器协同启用，不把图语义能力绑定在单一关系数据库扩展上。
...
## 4.3 端到端最小示例（Schema -> JoinWith -> 策略生效）
