...
关系语义支持等级：弱支持（以 `$lookup` / pipeline / 应用层预加载模拟关系语义）。

> vNext 方向：对外 API 将逐步收敛为后端无关的统一 `Query/Exec` 入口，由适配器自动映射到 Mongo 查询/写入/聚合计划；本文中的 Mongo 原生与自定义入口可视为当前阶段能力入口与兼容路径。
...
说明：以上属于当前兼容层行为。后续统一 API 改造会把入口聚焦到后端无关语义，不再要求用户显式区分 SQL/非 SQL 执行函数。
