...
// TestSQLServerDatabaseFeatures 测试 SQL Server 适配器声明的数据库特性
func TestSQLServerDatabaseFeatures(t *testing.T) {
	...
	execSQL(ctx, t, repo, `CREATE TABLE test_comp_idx (id INT, name VARCHAR(50), email VARCHAR(100))`)
	...
	// partial_indexes: SQL Server 支持筛选索引
	...
	// udt: SQL Server 支持用户定义类型
	execSQL(ctx, t, repo, "DROP TYPE IF EXISTS dbo.Point")
	assertFeatureSupported(ctx, t, repo, "udt",
		`CREATE TYPE dbo.Point FROM INT NOT NULL`)
	defer execSQL(ctx, t, repo, "DROP TYPE IF EXISTS dbo.Point")

	// stored_procedures: SQL Server 支持存储过程
	...
	// aggregate_funcs: SQL Server 聚合函数
	...
	// window_functions: SQL Server 支持窗口函数
	...
	// cte: SQL Server 支持 CTE
	assertQuerySupported(ctx, t, repo, "cte",
		`WITH numbered AS (SELECT 1 as n UNION ALL SELECT 2 UNION ALL SELECT 3)
		 SELECT * FROM numbered`)

	// recursive_cte: SQL Server 支持递归 CTE
	assertQuerySupported(ctx, t, repo, "recursive_cte",
		`WITH RECURSIVE numbers AS (
		   SELECT 1 as n
		   UNION ALL
		   SELECT n + 1 FROM numbers WHERE n < 10 AND n < 10
		 ) SELECT * FROM numbers`)

	// native_json: SQL Server 支持 JSON
	execSQL(ctx, t, repo, "DROP TABLE IF EXISTS test_json")
	assertFeatureSupported(ctx, t, repo, "native_json",
		`CREATE TABLE test_json (id INT, data NVARCHAR(MAX))`)
	defer execSQL(ctx, t, repo, "DROP TABLE IF EXISTS test_json")

	// json_path: SQL Server JSON_VALUE 和 JSON_QUERY
	execSQL(ctx, t, repo, `CREATE TABLE test_json2 (id INT, data NVARCHAR(MAX))`)
	execSQL(ctx, t, repo, `INSERT INTO test_json2 VALUES (1, N'{"name": "John", "age": 30}')`)
	assertQuerySupported(ctx, t, repo, "json_path",
		`SELECT JSON_VALUE(data, '$.name') FROM test_json2`)
	defer execSQL(ctx, t, repo, "DROP TABLE IF EXISTS test_json2")

	// json_index: SQL Server JSON 索引支持（通过计算列）
	execSQL(ctx, t, repo, "DROP TABLE IF EXISTS test_json_idx")
	execSQL(ctx, t, repo, `CREATE TABLE test_json_idx (
		id INT,
		data NVARCHAR(MAX),
		name AS JSON_VALUE(data, '$.name') PERSISTED
	)`)
	...
	// full_text_search: SQL Server 支持全文搜索
	execSQL(ctx, t, repo, "DROP TABLE IF EXISTS test_fts")
	execSQL(ctx, t, repo, `CREATE TABLE test_fts (id INT PRIMARY KEY, content NVARCHAR(MAX))`)
	execSQL(ctx, t, repo, `INSERT INTO test_fts VALUES (1, N'hello world'), (2, N'sqlserver database')`)
	assertQuerySupported(ctx, t, repo, "full_text_search",
		`SELECT * FROM test_fts WHERE CONTAINS(content, 'world')`)
	defer execSQL(ctx, t, repo, "DROP TABLE IF EXISTS test_fts")

	// generated: SQL Server 计算列
	...
	// returning: SQL Server OUTPUT 子句
	execSQL(ctx, t, repo, "DROP TABLE IF EXISTS test_returning")
	execSQL(ctx, t, repo, `CREATE TABLE test_returning (id INT PRIMARY KEY IDENTITY(1,1), name VARCHAR(50))`)
	assertQuerySupported(ctx, t, repo, "returning",
		`INSERT INTO test_returning (name) VALUES ('test') OUTPUT inserted.id, inserted.name`)
	defer execSQL(ctx, t, repo, "DROP TABLE IF EXISTS test_returning")

	// upsert: SQL Server MERGE 语句
	...
	assertFeatureSupported(ctx, t, repo, "upsert",
		`MERGE INTO test_upsert t USING (SELECT 1 as id, 'updated' as name) s
		 ON t.id = s.id
		 WHEN MATCHED THEN UPDATE SET name = s.name
		 WHEN NOT MATCHED THEN INSERT (id, name) VALUES (s.id, s.name)`)
	defer execSQL(ctx, t, repo, "DROP TABLE IF EXISTS test_upsert")

	t.Log("✓ SQL Server 所有数据库特性测试完成")
}

// TestSQLServerQueryFeatures 测试 SQL Server 适配器支持的查询特性
func TestSQLServerQueryFeatures(t *testing.T) {
	repo, cleanup := setupSQLServerRepo(t)
	if repo == nil {
		return
	}
	...
	// limit (SQL Server uses OFFSET...FETCH NEXT)
	assertQuerySupported(ctx, t, repo, "limit",
		`SELECT * FROM test_query_data ORDER BY id OFFSET 0 ROWS FETCH NEXT 2 ROWS ONLY`)

	// offset
	assertQuerySupported(ctx, t, repo, "offset",
		`SELECT * FROM test_query_data ORDER BY id OFFSET 1 ROWS FETCH NEXT 2 ROWS ONLY`)
	...
	// full_outer_join (SQL Server supports)
	...
	assertQuerySupported(ctx, t, repo, "cast",
		`SELECT id, CAST(value AS VARCHAR(10)) FROM test_query_data`)
	...
	assertQuerySupported(ctx, t, repo, "cte_in_query",
		`WITH sorted AS (SELECT TOP 3 * FROM test_query_data ORDER BY value DESC)
		 SELECT * FROM sorted`)
